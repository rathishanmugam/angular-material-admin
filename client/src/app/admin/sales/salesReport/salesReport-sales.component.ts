...
@Component({
  selector: 'app-salesreport-sales',
  templateUrl: './salesReport-sales.component.html',
  styleUrls: ['./salesReport.component.scss']
})
export class SalesReportSalesComponent implements OnInit, AfterViewInit {
  displayedColumns: string[] = ['billNo', 'salesDate', 'customerName', 'address', 'phoneNo', 'totalNetAmount', 'billType', 'delivered', 'details'];// 'serialNo', 'modelNo', 'qty', 'totalRate', 'billType', 'financeName', 'loanAmount'];
  ...
  getUsers() {
    this.loading = true;
    this.dataService
      .getSales(0, 5, 'companyName', 1, '').pipe(throttleTime(50000), catchError(() => of([])),
      finalize(() => this.loading = false))
      .subscribe((result: any) => {
          this.total = result.count;
          this.sales = result.docs;
          console.log('total', this.total);
          console.log('product', this.sales);
  ...
  loadRecordsPage() {
    ...
    this.dataService.getSales(this.paginator.pageIndex, this.paginator.pageSize, this.sort.active,
  ...
  productDetails(element: ISales): void {
    console.log('the element is ========>', element);
    this.dataService.changeSelectedBillNo(element);
